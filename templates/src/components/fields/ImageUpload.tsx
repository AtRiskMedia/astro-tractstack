...
import type { ImageFileNode } from '@/types/compositorTypes';
...
export const ImageUpload = ({
  ...
  const compressImage = async (file: File) => {
    ...
    try {
      // TODO: use backend for image compression
      console.log(`TODO: use backend for image compression`);
      return file;
    } catch (error) {
      ...
      if (file.type === 'image/svg+xml') {
        ...
        const processedImages = await Promise.all(
          TARGET_WIDTHS.map(async (width) => {
            ...
            if (!response.ok)
              throw new Error(`Failed to save ${width}px image`);
        ...
        const mainImagePath = processedImages.find(
          (img) => img.width === 1920
        )?.path;
        if (!mainImagePath) throw new Error('Failed to generate main image');
        const srcSet = processedImages
  ...
  return (
    <div ref={comboboxRef} className="flex items-center space-x-4">
      ...
      <div className="bg-mylightgrey/5 relative aspect-video w-40 overflow-hidden rounded-md border-2 bg-slate-50">
        <img
          src={currentImage}
          alt=""
        ...
        {currentFileId && (
          <button
            onClick={onRemove}
            className="hover:bg-mylightgrey absolute right-1 top-1 rounded-full bg-white p-1 shadow-md"
          >
      ...
      {isProcessing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="w-full max-w-md rounded-lg bg-white p-6 text-center">
            <h3 className="mb-4 text-xl font-bold">Processing Image</h3>
            <div className="mb-4 animate-pulse">
              <div className="h-2 rounded bg-cyan-600"></div>
            </div>
            <p className="text-mydarkgrey text-lg">{processingStep}</p>
          </div>
        </div>
      )}
